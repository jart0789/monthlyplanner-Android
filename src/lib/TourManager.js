...
// --- 1. EXIT CONFIRMATION HANDLER ---
const confirmExitHandler = (e) => {
  // Check if the clicked element is the driver close button (the "X")
  if (e.target.closest('.driver-popover-close-btn')) {
    if (!confirm("You are about to close this tutorial are you sure?")) {
      // CANCEL: Stop the close action, keep tour open
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation();
    } else {
      // OK: User confirmed exit
      // 1. Mark as complete (User Requirement)
      localStorage.setItem('fintracker_tutorial_completed', 'true');
      
      // 2. Force Immediate Cleanup to unlock screen
      TourManager.cleanup();
    }
  }
};
...
// --- 3. LOCK UI (CSS Injection) ---
const enableTourMode = () => {
  document.body.classList.add('tour-ui-locked');

  // Add listeners
  document.addEventListener('click', confirmExitHandler, true); // Capture phase
  document.addEventListener('keydown', preventEscHandler, true);
  ...
  style.innerHTML = `
    ...
    /* 2. STRICTLY DISABLE the element being highlighted. [CRITICAL FIX]
       Driver.js automatically sets this to 'auto', so we must use !important 
       to override it and keep the button unclickable. */
    ...
    /* 3. Re-enable the Popover so Next/Back/X buttons work */
    .driver-popover,
    .driver-popover * {
...
// --- 4. UNLOCK UI ---
const disableTourMode = () => {
  document.body.classList.remove('tour-ui-locked');
  
  document.removeEventListener('click', confirmExitHandler, true);
  document.removeEventListener('keydown', preventEscHandler, true);

  const style = document.getElementById('tour-interaction-lock');
  if (style) style.remove();

  // Safety: Manually reset body pointer-events just in case
...
export const TourManager = {
  ...
  run: (pageName, onNavigate, t, localActions = {}) => {
    ...
    const commonConfig = {
        ...
        // --- CLOSING LOGIC ---
        allowClose: true,  // Show the "X" button
        overlayClickBehavior: () => {}, // Do NOTHING if user clicks the grey overlay
        // ---------------------
        ...
        // Safety: If the tour ends naturally (Done button), unlock the UI
        onDestroy: () => {
            disableTourMode();
        }
    };
    ...
    const stepsConfig = {
      ...
      income: [
        { 
            ...
            popover: { 
                title: t('tour_add_income_title'), 
                description: t('tour_add_income_desc'),  
                side: "top", align: 'end',
                onPrevClick: () => TourManager.switchTab('dashboard', onNavigate)
            } 
        },
        { 
            element: '.tour-list-months', 
            popover: { 
                title: t('tour_list_months_title'), 
                description: t('tour_list_months_desc'),    
                onNextClick: () => {
                   if (localActions.setTab) {
                       localActions.setTab('analysis');
                       setTimeout(() => { if(driverObj) driverObj.moveNext(); }, 800);
                   } else {
        ...
        { 
            element: '.tour-YearlyTotal', 
            popover: { 
                title: t('tour_inc_yearlytotal_title'), 
                description: t('tour_inc_yearlytotal_desc'),
                onPrevClick: () => {
                   if (localActions.setTab) {
                       localActions.setTab('list');
                       setTimeout(() => { if(driverObj) driverObj.movePrevious(); }, 800);
                   } else {
                       if(driverObj) driverObj.movePrevious();
                   }
                } 
            } 
        }, 
        ...
        { 
            element: '.tour-YearlyDetail', 
            popover: { 
                title: t('tour_inc_yearlydetail_title'), 
                description: t('tour_inc_yearlydetail_desc'),  
                onNextClick: () => {
                   if (localActions.setTab) {
                       localActions.setTab('list');
                       setTimeout(() => { if(driverObj) driverObj.moveNext(); }, 800);
        ...
        { 
            element: '.tour-list-total', 
            popover: { 
                title: t('tour_totalincome_title'), 
                description: t('tour_totalincome_desc'),
                onPrevClick: () => {
                   if (localActions.setTab) {
                       localActions.setTab('analysis');
                       setTimeout(() => { if(driverObj) driverObj.movePrevious(); }, 800);
      ...
      expenses: [
        { 
            element: '.tour-add-btn', 
            popover: { 
                title: t('tour_add_expense_title'), 
                description: t('tour_add_expense_desc'),  
                side: "top", align: 'end',
                onPrevClick: () => TourManager.switchTab('income', onNavigate)
            } 
        },
        { 
            element: '.tour-list-months', 
            popover: { 
                title: t('tour_exp_list_months_title'), 
                description: t('tour_exp_list_months_desc'),    
                onNextClick: () => {
                   if (localActions.setTab) {
                       localActions.setTab('analysis');
                       setTimeout(() => { if(driverObj) driverObj.moveNext(); }, 800);
        ...
        { 
            element: '.tour-YearlyTotal', 
            popover: { 
                title: t('tour_exp_yearlytotal_title'), 
                description: t('tour_exp_yearlytotal_desc'),
                onPrevClick: () => {
                   if (localActions.setTab) {
                       localActions.setTab('list');
                       setTimeout(() => { if(driverObj) driverObj.movePrevious(); }, 800);
        ...
        { 
            element: '.tour-YearlyDetail', 
            popover: { 
                title: t('tour_exp_yearlydetail_title'), 
                description: t('tour_exp_yearlydetail_desc'),  
                onNextClick: () => {
                   if (localActions.setTab) {
                       localActions.setTab('list');
                       setTimeout(() => { if(driverObj) driverObj.moveNext(); }, 800);
        ...
        { 
            element: '.tour-list-total', 
            popover: { 
                title: t('tour_totalexpense_title'), 
                description: t('tour_totalexpense_desc'),
                onPrevClick: () => {
                   if (localActions.setTab) {
                       localActions.setTab('analysis');
                       setTimeout(() => { if(driverObj) driverObj.movePrevious(); }, 800);
      ...
      credits: [
        { 
            element: '.tour-add-credit', 
            popover: { 
                title: t('tour_add_debt_title'), 
                description: t('tour_add_debt_desc'),
                onPrevClick: () => TourManager.switchTab('expenses', onNavigate)
            } 
        },
        ...
        { 
            ...
            popover: { 
                ...
                onNextClick: () => TourManager.switchTab('snapshot', onNavigate)
            }
        },
        dummyStep
      ],
      snapshot: [
        { 
            popover: { 
                title: t('tour_snapshot_title'), 
                description: t('tour_snapshot_desc'),
                onPrevClick: () => TourManager.switchTab('credits', onNavigate) 
            } 
        },
      ...
      settings: [
        { 
            popover: { 
                title: t('tour_settings_title') || 'Settings', 
                description: t('tour_settings_desc') || 'Customize your experience here.',
                onPrevClick: () => TourManager.switchTab('snapshot', onNavigate)
            } 
        },
        { 
        ...
        { 
            element: '.tour-categories-name', 
            popover: { 
                title: t('tour_cat_name') || 'Category Name', 
                description: t('tour_cat_name_desc') || 'Provide a name for your category.',
                onPrevClick: () => {
                   if (localActions.setTab) {
        ...
        { 
            ...
            popover: { 
                title: t('tour_dark_mode_title') || 'Dark Mode', 
                description: t('tour_dark_mode_desc') || 'Switch between light and dark themes.',
                onPrevClick: () => {
                   if (localActions.setTab) {
                       localActions.setTab('categories');
                       setTimeout(() => { if(driverObj) driverObj.movePrevious(); }, 800);
