...
import { Moon, Sun, Globe, DollarSign, Plus, Trash2, Edit2, Check, Bell, CreditCard, RefreshCw } from 'lucide-react';
...
export default function Settings() {
  ...
  const resetForm = () => {
    setCatForm({ name: '', type: 'expense', color: '#3B82F6', icon: 'Tag', notificationsEnabled: false });
    setIsEditing(false);
    setEditingId(null);
  };
  ...
  const handleSaveCategory = () => {
    ...
    if (isEditing && editingId) {
      updateCategory(editingId, catForm);
    } else {
      addCategory(catForm.name, catForm.type, catForm.icon, catForm.color, catForm.notificationsEnabled);
    }
    resetForm();
  };
    ...
    <div className="space-y-6 pb-24 animate-in fade-in">
      ...
      <div className="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-xl">
        <button 
          ...
          className={cn("flex-1 py-2 rounded-lg text-sm font-bold transition-all", activeTab === 'general' ? "bg-white dark:bg-slate-700 shadow-sm text-slate-900 dark:text-white" : "text-slate-500")}
        >
          General
        </button>
        <button 
          onClick={() => setActiveTab('categories')}
          className={cn("flex-1 py-2 rounded-lg text-sm font-bold transition-all", activeTab === 'categories' ? "bg-white dark:bg-slate-700 shadow-sm text-slate-900 dark:text-white" : "text-slate-500")}
        >
          Categories
        </button>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700 flex justify-between items-center">
            ...
            <button 
              ...
              className={cn("w-12 h-7 rounded-full transition-colors relative", settings.theme === 'dark' ? "bg-indigo-500" : "bg-slate-300")}
            >
          ...
          <div className="space-y-3">
             ...
             {/* 1. Category Reminders */}
             <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700 flex justify-between items-center">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-indigo-100 dark:bg-slate-700 text-indigo-600 rounded-lg">
                ...
                <button 
                  onClick={() => updateNotificationSetting('bill_reminders', !getNotifState('bill_reminders'))}
                  className={cn("w-12 h-7 rounded-full transition-colors relative", getNotifState('bill_reminders') ? "bg-indigo-600" : "bg-slate-300")}
                >
             ...
             <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
                <div className="flex justify-between items-center mb-3">
                    ...
                    <button 
                      onClick={() => updateNotificationSetting('loan_dates', !getNotifState('loan_dates'))}
                      className={cn("w-12 h-7 rounded-full transition-colors relative", getNotifState('loan_dates') ? "bg-blue-600" : "bg-slate-300")}
                    >
                ...
                <div className={cn("flex items-center justify-between pt-3 border-t border-slate-100 dark:border-slate-700 transition-opacity", getNotifState('loan_dates') ? "opacity-100" : "opacity-40 pointer-events-none")}>
                    ...
                    <div className="flex items-center gap-2">
                        <input 
                            ...
                            min="0" 
                            max="30"
                            value={getNotifValue('loan_notify_days')}
                            onChange={(e) => updateNotificationSetting('loan_notify_days', parseInt(e.target.value))}
                            className="w-16 p-2 text-center bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg font-bold text-slate-900 dark:text-white outline-none focus:border-blue-500"
                        />
             ...
             <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700 flex justify-between items-center">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-emerald-100 dark:bg-slate-700 text-emerald-600 rounded-lg">
                ...
                <button 
                  onClick={() => updateNotificationSetting('autopay', !getNotifState('autopay'))}
                  className={cn("w-12 h-7 rounded-full transition-colors relative", getNotifState('autopay') ? "bg-emerald-600" : "bg-slate-300")}
                >
          ...
          <h4 className="text-xs font-bold text-slate-400 uppercase mt-4 ml-1">Regional</h4>
          ...
          <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700 flex justify-between items-center">
             ...
             <select 
               ...
               className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1 font-bold text-sm text-slate-700 dark:text-white outline-none"
             >
             ...
             <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                   <div>
                      ...
                      <input 
                        ...
                        className="w-full p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 font-bold text-slate-900 dark:text-white outline-none focus:border-blue-500"
                      />
                   </div>
                   <div>
                      ...
                      <div className="flex p-1 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700">
                         <button 
                           ...
                           className={cn("flex-1 py-2 rounded-lg text-xs font-bold transition-all", catForm.type === 'expense' ? "bg-white dark:bg-slate-700 shadow-sm text-rose-500" : "text-slate-400")}
                         >
                           Expense
                         </button>
                         <button 
                           onClick={() => setCatForm({...catForm, type: 'income'})}
                           className={cn("flex-1 py-2 rounded-lg text-xs font-bold transition-all", catForm.type === 'income' ? "bg-white dark:bg-slate-700 shadow-sm text-emerald-500" : "text-slate-400")}
                         >
                           Income
                         </button>
                      </div>
                ...
                {catForm.type === 'expense' && (
                  <div className="bg-slate-50 dark:bg-slate-900 p-3 rounded-xl flex justify-between items-center border border-slate-200 dark:border-slate-700">
                    ...
                    <button 
                      onClick={() => setCatForm({...catForm, notificationsEnabled: !catForm.notificationsEnabled})}
                      className={cn("w-10 h-6 rounded-full transition-colors relative", catForm.notificationsEnabled ? "bg-indigo-500" : "bg-slate-300")}
                    >
                   ...
                   <div className="flex flex-wrap gap-2">
                      {COLOR_OPTIONS.map(c => (
                        <button 
                          ...
                          className={cn("w-8 h-8 rounded-full transition-transform hover:scale-110 flex items-center justify-center", catForm.color === c ? "ring-2 ring-offset-2 ring-slate-400 dark:ring-offset-slate-800" : "")}
                ...
                <div className="flex gap-3 pt-2">
                   ...
                   <button 
                     ...
                     className="flex-[2] py-3 bg-blue-600 text-white font-bold rounded-xl shadow-lg shadow-blue-500/30 flex items-center justify-center gap-2"
                   >
