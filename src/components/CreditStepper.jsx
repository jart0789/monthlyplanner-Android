...
export default function CreditStepper({ onClose, initialData = null }) {
  ...
  const handleNext = () => {
    ...
    if (step === 2 && (!formData.totalAmount || !formData.currentBalance)) return alert("Please enter amounts");
    setStep(s => s + 1);
  };
  ...
  const handleFinish = () => {
    if (!formData.minPayment) return alert("Please enter minimum payment");
