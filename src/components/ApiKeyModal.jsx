      ...
      <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-700 flex flex-col max-h-[90vh] relative">
            ...
            <div className="absolute inset-0 z-50 bg-slate-900/95 backdrop-blur-md flex items-center justify-center p-6 animate-in zoom-in-95 duration-300">
                <div className="w-full bg-white dark:bg-slate-800 rounded-2xl p-1 shadow-2xl border border-slate-700 relative">
                    ...
                    <div className="p-5 text-center space-y-4">
                        ...
                        <h3 className="text-xl font-bold text-slate-800 dark:text-white">{t('connected_success')}</h3>
                        ...
                        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-900 p-4 rounded-xl flex gap-3 text-left">
                            ...
                            <div className="text-xs text-amber-800 dark:text-amber-200 leading-relaxed">
                                <strong>{t('note')}:</strong> {t('free_tier')}  
                                <ul className="list-disc pl-4 mt-1 space-y-1">
                                    <li>{t('limit')}<strong>{t('limit_amount')}</strong>.</li>
                        ...
                        <p className="text-xs text-slate-400 animate-pulse">{t('closed_in_seconds')}</p>
        ...
        <div className="bg-indigo-600 p-5 flex-none">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h2 className="text-lg font-bold text-white">{t('connect_ai_title')}</h2>
                    <p className="text-indigo-100 text-xs">{t('connect_ai_subtitle')}</p>
                </div>
            </div>
        </div>
        ...
        <div className="p-5 overflow-y-auto space-y-5">
            ...
            <div className="space-y-3">
                ...
                <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4 text-sm text-slate-600 dark:text-slate-300 space-y-3 border border-slate-100 dark:border-slate-700">
                    ...
                    <div className="flex gap-3">
                        ...
                        <p>{t('step_2_1')} <strong className="text-indigo-600 dark:text-indigo-400">{t('step_2_2')}</strong> {t('step_2_3')} <strong className="text-indigo-600 dark:text-indigo-400">{t('step_2_4')}</strong>.</p>
                    </div>
                    <div className="flex gap-3">
            ...
            >
                <div className="flex flex-col">
                    <span className="text-sm font-bold text-indigo-700 dark:text-indigo-300">{t('tap_to_get_key')}</span>
                    <span className="text-xs text-indigo-500/80">{t('opens_google')}</span>
                </div>
            ...
            <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-500">{t('paste_key_here')}</label>
                <input 
                    value={apiKey}
            ...
            <div className="space-y-3 pt-2">
                ...
                >
                    {isSaving ? t('Connecting') : t('connect_my_assistant')} 
